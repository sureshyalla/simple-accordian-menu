...
(function ($) {
	...
	$.fn.accordian=function(o)
  {
		...
		var settings = $.extend({},defaults,o)
		var accordian=$(this);
		var accordianmain=accordian.find('div');
		accordianmain.find('h3').next('div').hide();
		 
		if(settings.first==true){
			 
		 accordianmain.find('h3').eq(0).next('div').slideDown(settings.slidespeed);
		}
		accordianmain.find('h3').click(function()
		{
			if(settings.slide=='toggle'){
			 if($(this).next('div').is(":visible"))
			 {
		     slidetoglle($(this));	
			 }else{
			accordianmain.find('div').slideUp(settings.slidespeed);
			accordianmain.find('span').removeClass('active')	 
			slidetoglle($(this));	
			}
			}else{
			slidetoglle($(this));	
		...
		function slidetoglle(slidval)
		{
			slidval.next('div').slideToggle(settings.slidespeed);
			slidval.find('span').toggleClass('active')
		}
